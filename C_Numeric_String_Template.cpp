...
void solve(){
 ...
 vl k;
 rep(i,0,n){
    if(ma[a[i]].size()==0) {ma[a[i]].PB(i);k.PB(a[i]);}
    else{
        ma[a[i]].PB(i);
    }
 }
 rep(i,0,m){
    map<char,vector<ll>> ms;
     string s;
     cin>>s;
     if(s.size()!=n){
        cout<<"NO\n";
 ...
 aff(ks)
    aff(k)
     ...
     bool test=true;
     rep(i,0,ks.size()) {
        if(ms[ks[i]].size()!=ma[k[i]].size()){test=false;break;}
        rep(j,0,ms[ks[i]].size()){
            if(ms[ks[i]][j]!=ma[k[i]][j]){test=false;break;}
        }
        if(!test)break;
     }
