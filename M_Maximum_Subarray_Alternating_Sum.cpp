...
cin>>a;
   ...
   long long positive_sum = a[0];  // Assume the first element is positive
    long long negative_sum = 0;     // Start with zero for negative contributions
    long long max_sum = positive_sum; // Initialize the max sum

    // Traverse the array
    for (int i = 1; i < n; ++i) {
        // Alternating between positive and negative contributions
        if (i % 2 == 1) {
            // i is odd, treat a[i] as a negative contribution
            negative_sum = max(positive_sum - a[i], (long long) -a[i]);
            max_sum = max(max_sum, negative_sum);
        } else {
            // i is even, treat a[i] as a positive contribution
            positive_sum = max(negative_sum + a[i], (long long)a[i]);
